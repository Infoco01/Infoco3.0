...
import { PlusCircle, Upload, FileText, Download, Trash2, Info, Loader2 } from 'lucide-react';
...
const NotesTab: React.FC = () => {
    ...
    const handleSaveNote = async () => {
        ...
        try {
            const response = await fetch(`/api/upload-blob?filename=${encodeURIComponent(newNoteFile.name)}`, {
                method: 'POST',
                body: newNoteFile,
            });
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.error || 'Falha no upload do arquivo da nota.');
            }
