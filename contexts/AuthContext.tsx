...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updatePfp = (pfpUrl: string) => {
    if (user) {
        // If the old pfp exists and is a Vercel Blob URL, delete it.
        if (user.pfp && user.pfp.includes('.blob.vercel-storage.com')) {
            fetch('/api/delete-blob', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ url: user.pfp }),
            }).catch(err => console.error("Failed to delete old PFP from blob storage:", err));
        }
        
        const updatedUser = { ...user, pfp: pfpUrl };
        setUser(updatedUser);
        sessionStorage.setItem('infoco_user', JSON.stringify(updatedUser));
        localStorage.setItem(`infoco_user_pfp_${user.email}`, pfpUrl);
    }
  };
