...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateLoginScreenImage = (imageUrl: string | null) => {
    // If there's an old image and it's a blob URL, delete it from storage
    if (loginScreenImageUrl && loginScreenImageUrl.includes('.blob.vercel-storage.com')) {
        fetch('/api/delete-blob', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ url: loginScreenImageUrl }),
        }).catch(err => console.error("Failed to delete old login image from blob storage:", err));
    }
  ...
  const updateMunicipality = (updatedMunicipality: FinanceData) => {
    setFinanceData(prev => {
        const oldData = prev.find(m => m.id === updatedMunicipality.id);
        // If the URL has changed and the old one was a blob, delete the old one.
        if (oldData?.coatOfArmsUrl && oldData.coatOfArmsUrl !== updatedMunicipality.coatOfArmsUrl && oldData.coatOfArmsUrl.includes('.blob.vercel-storage.com')) {
            fetch('/api/delete-blob', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ url: oldData.coatOfArmsUrl }),
            }).catch(err => console.error("Failed to delete old coat of arms from blob storage:", err));
        }
        return prev.map(m => m.id === updatedMunicipality.id ? updatedMunicipality : m)
    });
  };
  ...
  const deleteFile = (municipality: string, folder: string, fileId: number) => {
    setDocuments(prev => {
        const fileToDelete = prev[municipality]?.[folder]?.find(f => f.id === fileId);
        // If the file exists and has a blob URL, delete it from storage
        if (fileToDelete?.url && fileToDelete.url.includes('.blob.vercel-storage.com')) {
            fetch('/api/delete-blob', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ url: fileToDelete.url })
            }).catch(err => console.error('Failed to delete blob from storage:', err));
        }
    ...
    const deletePaymentNote = (municipality: string, noteId: number) => {
        setPaymentNotes(prev => {
            const noteToDelete = prev[municipality]?.find(n => n.id === noteId);
            if (noteToDelete?.file?.url && noteToDelete.file.url.includes('.blob.vercel-storage.com')) {
                fetch('/api/delete-blob', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ url: noteToDelete.file.url })
                }).catch(err => console.error('Failed to delete payment note blob:', err));
            }
            const newNotes = { ...prev };
            if (newNotes[municipality]) {
